 ...
 * <li>Calling Base64.setFormat(Base64.BASE64_FORMAT.ORDERED_FORMAT) generates URL and file name friendly format that preserves lexical ordering as described
 ...
 * <li>v2.0 - I got rid of methods that used booleans to set options. Now everything is more consolidated and cleaner. The code now detects when data that's
 ...
 * <li>v1.5.1 - Fixed bug when decompressing and decoding to a byte[] using <tt>decode( String s, boolean gzipCompressed )</tt>. Added the ability to
 * "suspend" encoding in the Output Stream so you can turn on and off the encoding if you need to embed base64 data in an otherwise "normal" stream (like an XML
 * file).</li>
...
public class Base64
{
	...
	/**
	 * Returns one of the _SOMETHING_ALPHABET byte arrays depending on the options specified. It's possible, though silly, to specify ORDERED and URLSAFE in
	 * which case one of them will be picked, though there is no guarantee as to which one will be picked.
	 */
	...
	/**
	 * Returns one of the _SOMETHING_DECODABET byte arrays depending on the options specified. It's possible, though silly, to specify ORDERED and URL_SAFE in
	 * which case one of them will be picked, though there is no guarantee as to which one will be picked.
	 */
	 ...
	 * bytes in your array is given by <var>numSigBytes</var>. The array <var>threeBytes</var> needs only be as big as <var>numSigBytes</var>. Code can reuse
	 * a byte array by passing a four-byte array as <var>b4</var>.
	 * 
	 * @param b4
	...
	/**
	 * Serializes an object and returns the Base64-encoded version of that serialized object. If the object cannot be serialized or there is another error, the
	 * method will return <tt>null</tt>. The object is not GZip-compressed before being encoded.
	 * 
	...
	/**
	 * A {@link Base64.InputStream} will read data from another <tt>java.io.InputStream</tt>, given in the constructor, and encode/decode to/from Base64
	 * notation on the fly.
	 * 
	 * @see Base64
	 * @since 1.3
	...
	public static class InputStream extends java.io.FilterInputStream
	{
		...
		/**
		 * Calls {@link #read()} repeatedly until the end of stream is reached or <var>len</var> bytes are read. Returns number of bytes read into array or -1
	...
	/**
	 * A {@link Base64.OutputStream} will write data to another <tt>java.io.OutputStream</tt>, given in the constructor, and encode/decode to/from Base64
	 * notation on the fly.
	 * 
	 * @see Base64
	 * @since 1.3
	...
	public static class OutputStream extends java.io.FilterOutputStream
	{
		...
		/**
		 * Writes the byte to the output stream after converting to/from Base64 notation. When encoding, bytes are buffered three at a time before the output
		 * stream actually gets a write() call. When decoding, bytes are buffered four at a time.
		 * 
		 * @param theByte
		 *            the byte to write
		 * @since 1.3
		...
		public void write(int theByte) throws java.io.IOException
		{
			...
			else
			{
				...
				else if (decodabet[theByte & 0x7f] != WHITE_SPACE_ENC) { throw new java.io.IOException("Invalid character in Base64 data."); } // end else: not
																																				// white space
																																				// either
			} // end else: decoding
		} // end write
